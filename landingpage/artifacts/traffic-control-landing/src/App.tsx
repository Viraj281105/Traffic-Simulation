...
const capabilities: { icon: IconComponent; title: string; body: string }[] = [
  { icon: CarFront, title: 'IDM realistic vehicle physics', body: 'Every vehicle reacts to its leader, its gap, and the road — not a spreadsheet average.' },
  { icon: GitCompareArrows, title: 'Dual control strategies', body: 'Run fixed-time signals and modern roundabouts against the same demand.' },
  { icon: RadioTower, title: 'Real-time visualization', body: 'Watch queues form, dissolve, and negotiate space in a living intersection.' },
  { icon: ChartNoAxesCombined, title: 'Comprehensive analytics', body: 'Translate movement into evidence your next design review can use.' },
];
  ...
  return (
    <main className="site-shell">
        ...
        <div className="hero-grid">
          <div className="hero-copy">
            ...
            <div className="hero-actions">
              <a className="primary-btn" href="#compare" data-testid="link-explore-simulation">Explore simulation <ArrowDownRight size={15} /></a>
          ...
          <div className="hero-visual" aria-label="Animated isometric intersection visualization">
            ...
            <div className="visual-label top mono">SCENARIO_04 / DEMAND 1.00</div>
            <div className="visual-label bottom mono">● LIVE TWIN / T+ 00:43:18</div>
          </div>
        </div>
          ...
          <div className="capability-grid">
            {capabilities.map(({ icon: Icon, title, body }, index) => (
              <article className="capability" key={title} data-testid={`card-capability-${index}`}>
                <div className="capability-topline"><span className="capability-index mono">0{index + 1}</span><div className="capability-icon"><Icon size={19} strokeWidth={1.5} /></div></div>
                <h3>{title}</h3>
                <p>{body}</p>
              </article>
            ))}
      ...
      <section className="section-wrap final-cta reveal" aria-labelledby="final-title">
        ...
        <a className="primary-btn" href="#top" data-testid="link-run-another-scenario">Explore simulation <ArrowRight size={15} /></a>
      </section>
