...
const MOCK_SESSION = {
  ...
  curves: {
    ...
    signal: { delays: [5, 8, 14], throughputs: [100, 95, 80], queues: [1, 3, 7] },
    roundabout: { delays: [3, 9, 15], throughputs: [105, 90, 75], queues: [0.5, 2.5, 8] },
...
describe("VolumeAnalysisDashboard", () => {
  ...
  it("renders the trigger panel with Run Sweep button", () => {
    ...
    expect(screen.getByText(/Run Volume Sweep Experiment/i)).toBeInTheDocument();
  ...
  it("displays crossover badge when a sweep session is active", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/Critical Saturation Crossover/i)).toBeInTheDocument()
    );
