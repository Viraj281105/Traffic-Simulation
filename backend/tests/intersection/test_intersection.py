...
def test_lane_following_with_shared_connection_lanes() -> None:
    ...
    v1 = Vehicle(
        "v1", length=4.0, width=2.0, desired_speed=10.0,
        route=route, start_position=50.0, initial_speed=10.0,
        turn_intent=TurnIntent.STRAIGHT,
    )
    v2 = Vehicle(
        "v2", length=4.0, width=2.0, desired_speed=10.0,
        route=route, start_position=30.0, initial_speed=10.0,
        turn_intent=TurnIntent.STRAIGHT,
    )
...
def test_emergency_proximity_detection() -> None:
    ...
    v1 = Vehicle(
        "v1", length=4.0, width=2.0, desired_speed=10.0,
        route=[lane], start_position=10.0, initial_speed=5.0,
    )
    v2 = Vehicle(
        "v2", length=4.0, width=2.0, desired_speed=10.0,
        route=[lane], start_position=6.0, initial_speed=5.0,
    )
...
def test_conflict_manager_full_lifecycle_and_arbitration() -> None:
    ...
    lane_unrelated = Lane("conn_c", start_x=100.0, start_y=100.0, end_x=120.0, end_y=100.0)
    ...
    assert cm.get_conflict_distance("v1", TurnIntent.STRAIGHT, "conn_unknown", 0.0, 0.0) == float("inf")
    ...
    dist = cm.get_conflict_distance("v_left", TurnIntent.LEFT, "conn_a", 10.0, 0.0, all_vehicles_info=all_info_1)
    assert dist < float("inf")

    # Now query v_straight on conn_b (other vehicle v_left on conn_a == cp.lane_id_a)
    dist_straight = cm.get_conflict_distance("v_straight", TurnIntent.STRAIGHT, "conn_b", 10.0, 0.0, all_vehicles_info=all_info_1)
    ...
    dist_tie = cm.get_conflict_distance("v1_beta", TurnIntent.STRAIGHT, "conn_a", 10.0, 0.0, all_vehicles_info=all_info_2)
    assert dist_tie < float("inf")

    # 3. v0_alpha acquires reservation
    dist_acquire = cm.get_conflict_distance("v0_alpha", TurnIntent.STRAIGHT, "conn_b", 10.0, 0.0, all_vehicles_info=[])
    assert dist_acquire == float("inf")
    assert cm.get_reservation_count() == 1

    # Own reservation check (v0_alpha owns it)
    assert cm.get_conflict_distance("v0_alpha", TurnIntent.STRAIGHT, "conn_b", 10.0, 0.0) == float("inf")

    # Another vehicle yields to active reservation held by v0_alpha
    dist_blocked = cm.get_conflict_distance("v_other", TurnIntent.STRAIGHT, "conn_a", 10.0, 0.0)
    assert dist_blocked < float("inf")

    # Vehicle already past conflict point
    assert cm.get_conflict_distance("v_past", TurnIntent.STRAIGHT, "conn_a", 30.0, 0.0) == float("inf")
